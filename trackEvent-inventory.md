Google Analytics trackEvent Inventory — fishing-map (frontend monorepo)
...
- app/analytics.hooks.ts:52
  Action: 'general'
  Label: n/a (uses `other` with pagetype, language, user_login_state, user_id, organization_type, country, user_cohort, user_group)
...
- user/user.slice.ts:65
  Action: 'login'
  Label: n/a (uses `other` with user_id)
...
- NavigationHistoryButton.tsx:71
...
- workspace/vessel-groups/VesselGroupsSection.tsx:55
  ...
  Label: n/a (value: userDatasets.length)
...
- vessel/VesselHeader.tsx:75
  ...
  Label: `${vesselGroupId}` (value: number of vessel identities in group)
...
- vessel-groups/VesselGroupModal.tsx:427
  ...
  Label: getEventLabel([`vessel_id: ${vesselGroupId}`, calculateVMSVesselsPercentage(vesselGroupVessels)]) (value: number of vessels)
...
- search/SearchActions.tsx:87
  ...
  Label: `${activeSearchOption} search` (value: number of vessel added to group)
...
- map/map-interactions.hooks.ts (~line 305)
  Action: dynamic — dispatches getAnalyticsEvent(feature) result (not a static literal)
...
- timebar/Timebar.tsx:276
  Action: gaActions[e.source] (dynamic)
...
- reports/shared/vessels/ReportVesselsTableFooter.tsx:109
  ...
  Label: getEventLabel([`Groupd id: ${vesselGroup?.id}`, `start date: ${start}`, `end date: ${end}`]) (value: number of vessel identities)
...
- reports/report-vessel-group/VesselGroupReportLink.tsx:25
  ...
  Label: n/a (uses `other` with vesselGroupId)
