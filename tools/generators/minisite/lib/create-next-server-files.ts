...
export function createNextServerFiles(host: Tree, options: NormalizedSchema) {
  if (options.server) {
    const directory = dirname(joinPathFragments(options.appProjectRoot, options.server))
    ...
    host.write(
      ...
      /**
       * @typedef {import(${
         isOldNext ? 'next/dist/next-server/server/next-server' : 'next/dist/server/next-server'
       }).default} Server
       */
