...
Goal: produce a spec file (e.g. `.claude/specs/<feature>.plan.md`) that enumerates the scenarios to test. **Always** write the spec to a file.
...
### 1.3 Determine feature priority and tier budget

Do this **before** exploring — priority governs how deep to explore and how many edge cases are worth mapping at all. Tags are defined in `apps/platform-e2e/src/tags.ts`:

- `@smoke` (Tier 1) — happy paths of critical features only. Runs on every PR to staging/production, Chromium only, budget < 5 min.
- `@extended` (Tier 2) — more features and conditions. Runs before each release + daily, Chromium/Firefox/Webkit, budget < 30 min.
- `@regression` (Tier 3) — full suite with multiple edge cases per feature. Runs weekly + on-demand, + Mobile, budget > 45 min.

**Determine the feature's priority before assigning tiers.** If the user hasn't stated it, ask: P1 (critical — e.g. login/session/cookies, vessel search, dynamic report), P2 (medium — e.g. file upload, vessel identity visualization, encounter compliance/workspace isolation, permission-based layer addition), or P3 (low). Don't guess; a wrong priority miscalibrates every budget below it.

**Tier budget, per feature, driven by priority:**

- P1: `@smoke` gets 1 scenario, or 2 if the feature needs a second happy-path variant (e.g. guest vs. logged-in). `@extended` is capped at 2-3x that `@smoke` count.
- P2: no `@smoke` scenario. `@extended` gets 1-2 scenarios (the ones with zero fixture risk and full determinism — client-side validation, intercepted-response error handling, structural empty/no-results states); everything else goes to `@regression`.
- P3: no `@smoke` or `@extended` scenario — plan it entirely under `@regression`.
- `@regression` has no fixed cap.

Don't propose a `@regression`-only scenario as `@smoke`, and don't duplicate a flow already covered at a lower test level (unit/integration/API). Some features are already covered by e2e tests in the API repo, in addition to their unit and integration tests there — for those, do not add UI e2e coverage beyond a thin `@smoke` happy-path check (if the feature is critical); no `@extended`/`@regression` scenarios or edge-case coverage for logic already exercised there. This keeps the UI suite from growing unbounded with duplicate coverage.

Use known/fixed vessel IDs for search and track scenarios, and fixture files under `/tests/fixtures/` for uploads (valid CSV, valid KML, invalid format, malformed data) — no ad-hoc literals. Prefer reusing existing Page Object helpers (`apps/platform-e2e/src/pages/*`) over re-deriving locators; if a scenario needs a helper that doesn't exist yet, note it in the spec so Generate can add it.
...
### 1.5 Write the spec file

Save under `.claude/specs/<feature>.plan.md`. Use this structure:
...
<One paragraph: the feature's priority, the @smoke count and what it covers, the @extended
count and why each one qualifies (zero fixture risk, deterministic), and what's deferred to
@regression and why. This is what makes the tier split auditable later — see
`.claude/specs/dynamic-report.md` or `.claude/specs/vessel-search.md` for worked examples.>
...
#### 1.1. <kebab-case-scenario-name> `@smoke`
...
- Tag every scenario heading with exactly one tier (`@smoke`, `@extended`, or `@regression`) per the budget from 1.3 — this is what `Generate` uses to write each test's `test(..., { tag: ... })`.
- Cover happy path, edge cases, validation, negative flows, persistence — but only as deep as the tier budget allows; don't over-plan `@regression` detail for a P3 feature into `@extended`.
...
- **Spec file**, e.g. `.claude/specs/basic-operations.plan.md`.
...
Collect the generated code and write the test file at the path given in the spec. Carry the scenario heading's tier tag (`` `@smoke` ``/`` `@extended` ``/`` `@regression` ``, from [1.5](#15-write-the-spec-file)) into the test's `{ tag: [...] }` option, resolved through `TAGS` (`apps/platform-e2e/src/tags.ts`) rather than the literal string:

```ts
// spec: .claude/specs/basic-operations.plan.md
// seed: tests/seed.spec.ts
import { test, expect } from './fixtures' // or '@playwright/test' if no fixtures file
import { TAGS } from './tags'

test.describe('Signing in and out', () => {
  test('should sign in', { tag: [TAGS.SMOKE] }, async ({ page }) => {
...
- **Every test carries exactly the tier tag its scenario heading declares.** A scenario with no tag in its heading is a spec bug — stop and fix the spec (per 1.5) instead of guessing a tier.
