...
describe('Access to vessel viewver - search', () => {
  ...
  it(
    'should search for a vessel by mmsi "' + SEARCH_VESSEL_MMSI + '" and access to the profile in the search results',
    () => {
  ...
  it(
    'should search for a vessel by mmsi "' + SEARCH_VESSEL_MMSI + '" and access to the profile in the sidebar',
    () => {
      ...
      .then((vessel) => {
        ...
        .should('exist')
        cy.getBySel('link-vessel-profile').click({ force: true })
        cy.getBySel('vv-vessel-name').contains(vessel.text())
        verifyTracksInTimebar(3)
      })
    }
