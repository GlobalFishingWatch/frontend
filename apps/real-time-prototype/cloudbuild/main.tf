...
module "develop" {
  ...
  set_env_vars = [
    ...
    "BASIC_AUTH_USER=gfw-real",
  ]
  set_secrets = [
    "BASIC_AUTH_PASS=${local.secrets_path.dev}/BASIC_AUTH_PASS_REAL_TIME_PROTO",
  ]
}
...
#   ]
#   set_secrets = [
#     "BASIC_AUTH_PASS=${local.secrets_path.sta}/BASIC_AUTH_PASS_REAL_TIME_PROTO",
#   ]
# }
...
#   description       = "Deploy to production when pushing new tag @gfw/real-time-prototype@x.x.x"
#   app_name          = local.app_name
...
#   }
#   push_config = {
#     tag          = "^@gfw/real-time-prototype@\\d+\\.\\d+\\.\\d+(\\.\\d+)?$"
#     invert_regex = false
#   }
#   set_env_vars_build = [
