...
i18n is configured [here](apps/platform/features/i18n/i18n.ts) following this [spec](https://www.i18next.com/overview/configuration-options).
...
The source file, and its translations are located at the [locales folder](apps/platform/public/locales, served by HTTP and managed by [backend-plugin](https://github.com/i18next/i18next-http-backend).
...
```text
https://purge.jsdelivr.net/npm/@globalfishingwatch/i18n-labels@stable/en/flags.json
```

## Keys extraction + typescripts definitions

[i18n-parser](https://github.com/i18next/i18next-parser) script syncs the code and the translations extracting the keys to the `source/translations.json` file. It's configured [here](apps/platform/i18next-parser.config.js). Tu execute it, run:
...
- [`I18nFlag`](apps/platform/src/features/i18n/i18nFlag.tsx)
- [`I18nDate`](apps/platform/src/features/i18n/i18nDate.tsx) using [luxon formatting](https://moment.github.io/luxon/docs/class/src/datetime.js~DateTime.html)
- [`I18nNumber`](apps/platform/src/features/i18n/i18nNumber.tsx)
...
- when deploying to prod? on every deploy to prod of platform
