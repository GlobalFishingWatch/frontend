...
/**
 * Suffix marking a dataset-comparison dataview instance.
 * Mirrors `DATASET_COMPARISON_SUFFIX` in @globalfishingwatch/deck-layer-composer — declared here so
 * `data/config` does not have to re-export it, which pulled that whole package into the entry chunk
 * of every page. `libs/dataviews-client` keeps its own copy for the same reason.
 */
