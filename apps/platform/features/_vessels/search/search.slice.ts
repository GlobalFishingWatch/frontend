...
import { ADVANCED_SEARCH_FIELDS } from 'features/_vessels/search/advanced/advanced-search.utils'
...
import type { IdentityVesselData, VesselDataIdentity } from 'features/_vessels/vessel/vessel.slice'
import { getVesselId, getVesselIdentities } from 'features/_vessels/vessel/vessel.utils'
        ...
        const fields = andCombinedFields.flatMap((field) => {
          ...
          if (filter && isInFieldsAllowed) {
            let value = Array.isArray(filter)
              ...
              : (filter.trim() as string)
            // Supports searching by multiple values separated by comma and semicolon
            const regex = /[,;]/
            if (ADVANCED_SEARCH_FIELDS.includes(field as any) && regex.test(value as string)) {
