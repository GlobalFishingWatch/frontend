...
import { parseEventsToCSV } from 'features/vessels/vessel/vessel.download'
import { getVoyageTimeRange } from 'features/vessels/vessel/vessel.utils'
...
const VoyageGroup: React.FC<EventProps> = ({
  ...
  const voyageLabel = useMemo(() => {
    ...
    parts.push(
      `${eventCount} ${t((t) => t.common.events, {
        count: eventCount,
      })}`
    )
    parts.push(t((t) => t.common.between))
    parts.push(formatI18nDate(startDate, { format: DateTime.DATE_MED }))
    if (voyageStart) {
      parts.push(`(${voyageStart})`)
    }
    parts.push(t((t) => t.common.and))
        ...
        {hasEvents && (
          <div className={cx(styles.actions, 'print-hidden')}>
            ...
            <IconButton
              ...
              tooltip={t((t) => t.download.dataDownload)}
