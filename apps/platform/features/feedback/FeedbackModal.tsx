...
import { selectActiveDataviews } from 'features/map/dataviews/selectors/dataviews.selectors'
...
import { getWorkspaceReport, parseUpsertWorkspace } from 'features/map/workspace/workspace.utils'
import { createWorkspaceThunk } from 'features/map/workspaces-list/workspaces-list.slice'
...
const FEEDBACK_FEATURE_IDS = [
  'activityLayers' as const,
  'vesselTracks' as const,
  'vesselSearch' as const,
  'environmentLayers' as const,
  'referenceLayers' as const,
  'timebar' as const,
  'analysis' as const,
  'other' as const,
]
...
function FeedbackModal({ isOpen = false, onClose }: FeedbackModalProps) {
  ...
  const roleOptions = FEEDBACK_ROLE_IDS.map((roleId) => ({
    id: roleId,
    label: t((t) => t.feedback.roles[roleId], { defaultValue: roleId }),
  }))

  const featureOptions = FEEDBACK_FEATURE_IDS.map((featureId) => ({
    id: featureId,
    label: t((t) => t.feedback.features[featureId], { defaultValue: featureId }),
  }))
        ...
        if (
          ...
          if (isWorkspaceLocation) {
            ...
            url =
              ...
              PATH_BASENAME +
              // Workspaces live under /map; the standalone report below does not.
              `/map/${workspace?.category}/${workspace?.id}`
