...
import { selectVGRFootprintDataview } from 'features/_map/dataviews/selectors/dataviews.categories.selectors'
import {
  type ReportVesselWithDatasets,
  selectReportDataviewsWithPermissions,
} from 'features/_reports/report-area/area-reports.selectors'
import { getVesselsFiltered } from 'features/_reports/report-area/area-reports.utils'
import { selectVGRCoverageInsightData } from 'features/_reports/report-vessel-group/vessel-group-report.selectors'
import {
...
} from 'features/_reports/shared/utils/reports.utils'
import { REPORT_FILTER_PROPERTIES } from 'features/_reports/shared/vessels/report-vessels.config'
import { selectReportVesselsList } from 'features/_reports/tabs/activity/vessels/report-activity-vessels.selectors'
import { parseCoverageGraphValueBucket } from 'features/_reports/tabs/vessel-group-insights/vessel-group-report-insights.utils'
import { getVesselGroupUniqVessels } from 'features/_user/vessel-groups/vessel-groups.utils'
import type { VesselGroupVesselIdentity } from 'features/_user/vessel-groups/vessel-groups-modal.slice'
import { getSearchIdentityResolved, getVesselProperty } from 'features/_vessels/vessel/vessel.utils'
...
export function getVesselDatasetsWithoutRelated({
  vessels,
  vesselDatasets,
  relatedType,
}: {
  ...
  vessels?.forEach((vessel) => {
    ...
    const relatedDataset = getRelatedDatasetByType(infoDataset, relatedType)
    if (!relatedDataset) {
...
export const selectVGRVesselDatasetsWithoutInsightsRelated = createSelector(
  [selectVGRVessels, selectVesselsDatasets],
  (vessels = [], vesselDatasets) => {
    return getVesselDatasetsWithoutRelated({
      vessels,
      vesselDatasets,
      relatedType: DatasetTypes.Insights,
    })
  }
)
