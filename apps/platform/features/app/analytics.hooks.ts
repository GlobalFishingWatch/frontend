...
export const useAnalytics = () => {
  ...
  const locationType = useRouterState({
    select: (state) => {
      const fullPath = state.matches[state.matches.length - 1]?.fullPath
      return fullPath ? mapRoutePathToType(fullPath) : undefined
    },
  })
  const { category: locationCategory } = useParams({ strict: false })
