...
const workspaceModules = import.meta.glob<{ default: AppWorkspace }>(
  '../../../data/map/default-workspaces/workspace.*.ts'
)

if (!Object.keys(workspaceModules).length) {
  throw new Error(
    `No default workspaces matched "${DEFAULT_WORKSPACES_GLOB}/workspace.*.ts". ` +
      `The glob is relative to workspace.slice.ts — update it if either path moved.`
  )
}
...
export const getDefaultWorkspace = async (): Promise<AppWorkspace> => {
  ...
  const loader =
    workspaceModules[`${DEFAULT_WORKSPACES_GLOB}/workspace.${workspaceEnv}.ts`] ??
    workspaceModules[`${DEFAULT_WORKSPACES_GLOB}/workspace.production.ts`]
  if (!loader) {
    throw new Error(
      `No default workspace for env "${workspaceEnv}" and no production fallback. ` +
        `Available: ${Object.keys(workspaceModules).join(', ')}`
    )
  }
  ...
  async (
    ...
    try {
      ...
      const { getDatasetsInDataviews, getLatestEndDateFromDatasets, getVesselGroupsInDataviews } =
        await import('features/_map/datasets/datasets.utils')
      const { getVesselDataviewInstanceDatasetConfig } =
        await import('features/_map/dataviews/dataviews.utils')
      ...
      if (!signal.aborted) {
        ...
        if (vesselDataviewsWithTrack?.length) {
          ...
          const { mergeDataviewIntancesToUpsert } =
            await import('features/_map/workspace/workspace.hook')
...
const workspaceSlice = createSlice({
  ...
  extraReducers: (builder) => {
    // Logout cleanup. Lives here rather than as actions dispatched from user.slice's logout thunk so
    // that user.slice (eagerly registered) does not import this slice, which drags deck.gl and the
    // timebar into every page's entry chunk. Guarded by scripts/check-store-graph.mjs.
    // The logout thunk also used to dispatch a staff-only-dataview filter, but it ran after the
    // workspace data was already cleared, so it was a no-op. Clearing is the whole behaviour.
