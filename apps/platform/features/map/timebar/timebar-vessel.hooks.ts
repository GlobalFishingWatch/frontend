...
import { useTimebarVisualisationConnect } from 'features/map/timebar/timebar.hooks'
import { selectTimebarGraph } from 'features/map/workspace/selectors/app.timebar.selectors'
import { selectWorkspaceVisibleEventsArray } from 'features/map/workspace/workspace.selectors'
    ...
    requestAnimationFrame(() => {
      if (
        ...
        const vesselEvents: TimebarChartData<any> = vessels.map(({ instance }) => {
          ...
          const status = loaded ? ResourceStatus.Finished : ResourceStatus.Loading
          const dataview = trackDataviews?.find((d) => d.id === instance.id)
