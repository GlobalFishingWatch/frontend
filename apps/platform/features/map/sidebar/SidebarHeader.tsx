...
import NavigationHistoryButton from 'features/map/sidebar/buttons/NavigationHistoryButton'
import NavigationWorkspaceButton from 'features/map/sidebar/buttons/NavigationWorkspaceButton'
import ShareWorkspaceButton from 'features/map/sidebar/buttons/ShareWorkspaceButton'
import { selectHasTimeModeEnabled } from 'features/map/sidebar/sidebar.selectors'
import { getScrollElement } from 'features/map/sidebar/sidebar.utils'
import { selectReadOnly } from 'features/map/workspace/selectors/app.selectors'
import { selectWorkspaceHistoryNavigation } from 'features/map/workspace/workspace.selectors'
...
import {
  ...
  IMO_LENGTH,
  SSVID_LENGTH,
} from 'features/vessels/search/search.config'
import { selectSearchOption, selectSearchQuery } from 'features/vessels/search/search.config.selectors'
import { useSearchFiltersConnect } from 'features/vessels/search/search.hook'
 ...
 * across features/reports, features/vessel and features/workspace. Keeping them static made the
 * map-free shell pull the whole report and vessel graph. Measure with
 * `node scripts/reachable-features.mjs features/map/sidebar/SidebarHeader.tsx`.
 */
const ReportTitle = lazy(() => import('features/reports/report-area/title/ReportTitle'))
const PortReportHeader = lazy(() => import('features/reports/report-port/PortReportHeader'))
const VesselGroupReportTitle = lazy(
  () => import('features/reports/report-vessel-group/VesselGroupReportTitle')
)
const VesselHeader = lazy(() => import('features/vessels/vessel/VesselHeader'))
const SaveReportButton = lazy(() => import('features/map/sidebar/buttons/SaveReportButton'))
const SaveWorkspaceButton = lazy(() => import('features/map/sidebar/buttons/SaveWorkspaceButton'))
const TimeModeSelector = lazy(() => import('features/map/sidebar/TimeModeSelector'))
