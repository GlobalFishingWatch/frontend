...
import { getFinalDatasetFromMetadata } from 'features/map/datasets/upload/datasets-upload.utils'
import NewPointsDataset from 'features/map/datasets/upload/NewPointsDataset'
import NewPolygonDataset from 'features/map/datasets/upload/NewPolygonDataset'
import NewTrackDataset from 'features/map/datasets/upload/NewTrackDataset'
import { selectDataviewInstancesMerged } from 'features/map/dataviews/selectors/dataviews.resolvers.selectors'
import { RegisterOrLoginToUpload } from 'features/map/workspace/user/UserSection/UserSection'
    ...
    async (datasetMetadata, { file, isEditing } = {} as OnConfirmParams) => {
      if (datasetMetadata) {
        ...
        if (createDatasetError) {
          setError(`${t((t) => t.errors.generic)} ${SUPPORT_EMAIL}`)
        } else if (payload) {
          if (locationType === 'MAP' || locationType === 'WORKSPACE') {
    ...
    >
      {error ? (
        ...
        <Fragment>
          <p className={styles.instructions}>
            {style !== 'transparent'
              ? t((t) => t.dataset.dragAndDropFileToCreateDataset)
              : t((t) => t.dataset.dropFileToCreateDataset)}
          </p>
