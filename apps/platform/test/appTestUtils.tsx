...
function buildInitialHref(location: LocationState): string {
  ...
  const pathname = location.pathname?.startsWith(PATH_BASENAME)
    ? location.pathname
    : // MAP is '/map', not '/', so use the literal rather than assuming the root path.
      `${PATH_BASENAME}${location.to === ROUTE_PATHS.MAP ? ROUTE_PATHS.MAP : location.pathname || '/'}`
...
export async function render(options: AppRenderOptions = {}) {
  ...
  return page.mark('render app', async () => {
    ...
    // MAP is always the initial route — seeding is for the search params (lat/lng/zoom),
