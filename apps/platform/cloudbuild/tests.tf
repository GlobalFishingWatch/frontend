locals {
  e2e_trigger_configs = {
    staging = {
      name        = "platform-e2e-tests-staging"
    ...
    production = {
      name        = "platform-e2e-tests-prod"
      description = "Run end-to-end tests on tag @gfw/platform@x.x.x"
      push = {
        branch       = null
        tag          = "^@gfw/platform@\\d+\\.\\d+\\.\\d+(\\.\\d+)?$"
        invert_regex = false
      }
      env = {
  ...
  build {
    ...
    step {
      ...
      script = <<EOF
        npm install -g pnpm
        pnpm install --frozen-lockfile
        pnpm exec nx e2e platform-e2e -- --project="chromium" --no-cache
      EOF
      env = [
    ...
    step {
      ...
      args = ["-m", "cp", "-r", "apps/platform-e2e/test-results", "apps/platform-e2e/playwright-report", "gs://gfw-playwright-traces-ttl30/frontend/e2e-tests/$BUILD_ID/"]
    }
