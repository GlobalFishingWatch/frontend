...
export const resolveLibraryLayers = (
  ...
  const layers = LIBRARY_LAYERS.flatMap((layer) => {
    ...
    return {
      ...
      moreInfoLink: t(($) => $[layer.id].moreInfoLink, {
        ns: 'layer-library',
      }),
  ...
  if (experimentalLayers) {
    layers.push({
      ...
      name: t(($) => $['bathymetry-contour'].name, {
        ns: 'layer-library',
      }),
      description: t(($) => $['bathymetry-contour'].description, {
        ns: 'layer-library',
      }),
      moreInfoLink: '',
          ...
          {i18nReady &&
            extendedCategories.map(({ category, subcategories }) => (
              <div key={category}>
                ...
                >
                  {t((t) => t.common[category as DataviewCategory])}
                </button>
                {currentCategory === category &&
                  ...
                  subcategories.map((subcategory) => (
                    <button
                      ...
                      onClick={onCategoryClick}
                    >
                      {t((t) => t.dataset.type[upperFirst(subcategory)])}
                    </button>
                  ))}
              </div>
            ))}
