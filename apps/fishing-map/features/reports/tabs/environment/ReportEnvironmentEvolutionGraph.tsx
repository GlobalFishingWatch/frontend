...
function ReportEnvironmentEvolutionGraph() {
  useComputeReportTimeSeries()
  ...
  return (
    <Fragment>
      {environmentalDataviews.map((dataview, index) => {
        const isDynamic = isEnvironmentalDataview(dataview)
        const isCurrents = isHeatmapCurrentsDataview(dataview)
        const { min, mean, max } =
...
export default ReportEnvironmentEvolutionGraph
