...
module "staging" {
  ...
  set_secrets = [
    "BASIC_AUTH_PASS=${local.secrets_path.sta}/BASIC_AUTH_PASS_VESSEL_INFO_TOOL",
  ]
}
...
module "production" {
  ...
  description       = "Deploy to production when pushing new tag @gfw/vessel-info-tool@x.x.x"
  app_name          = local.app_name
  docker_image      = "us-central1-docker.pkg.dev/gfw-int-infrastructure/frontend/${local.app_name}:latest-sta"
  ...
  push_config = {
    tag          = "^@gfw/vessel-info-tool@\\d+\\.\\d+\\.\\d+(\\.\\d+)?$"
    invert_regex = false
  }
  set_env_vars_build = [
