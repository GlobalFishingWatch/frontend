...
export const fetchDataviewsByIdsThunk = createAsyncThunk(
  'dataviews/fetch',
  async (ids: number[] | string[], { signal, rejectWithValue, getState }) => {
  ...
  },
  {
    ...
    condition: (ids: number[] | string[], { getState }) => {
      const { dataviews } = getState() as RootState
      const fetchStatus = dataviews.status
      const slugs = Object.entries(dataviews.entities).map(([id, dataview]) => dataview.slug)
