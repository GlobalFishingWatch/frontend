...
function TableRow({ record, extra, onCountryChange }: TableRowProps) {
  const map = useMapInstance()
  const dispatch = useDispatch()
  const { t } = useTranslation()
  const { onSubareaChange, onPointValueChange, onPortChange } = useValueManagerConnect()
  const onRowHover = useCallback(
    (id: string, hover: boolean) => {
      ...
      map.setFeatureState({ source: 'pointsLayer', id: parseInt(id, 16) }, { hover })
    },
    [dispatch, map]
  )
  ...
  const subareaValues = useSelector(selectSubareaValuesByCountry) as
  ...
  const onSubareaAdded = useCallback(() => {
    ...
    const newId =
      Math.max.apply(
        null,
        subareas
          .map((subarea) => parseInt(subarea.id.replace(country + '-', '')))
  ...
  const onSubareaNameChange = useCallback(
  ...
  const onPortNameChange = useCallback(
    (id: string | number, value: string) => {
      dispatch(
        setPorts(
          ports.map((port) => ({
    ...
    >
      <div className={styles.col}>
        {country ? (
          ...
          <Tooltip content={record.port_label}>
      ...
      <div className={styles.col}>
        {country ? (
          ...
          <Tooltip content={record.community_label}>
      ...
      <div className={styles.col}>
        {country ? (
          ...
          <Tooltip content={record.point_label}>
            <span>{record.point_label}</span>
          </Tooltip>
        )}
      </div>
      <div className={styles.col}>
        <Tooltip content={record.top_destination}>
          <span>{record.top_destination}</span>
        </Tooltip>
      </div>
      {extra && (
        <div className={styles.col}>
      ...
      {extra && (
        <div className={styles.col}>
          <span onClick={() => onCountryChange(record)} className={styles.actionButton}>
            {flags[record.iso3 as keyof typeof flags] ?? record.iso3}
          </span>
        </div>
      )}
    </div>
