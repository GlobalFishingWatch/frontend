...
export const useReplaceLoginUrl = () => {
  ...
  const locationType = useSelector(getLocationType) as ROUTE_TYPES
  ...
  useEffect(() => {
    ...
    if (redirectUrl && currentQuery[CALLBACK_URL_PARAM]) {
      const redirectQuery = parse(new URL(redirectUrl).search)
      const query = {
        ...redirectQuery,
...
export const useGFWAuthentication = () => {
  const { loading, logged, error, user } = useGFWLogin(GFWAPI)
  if (!loading && !logged) {
    redirectToLogin()
  }
  return { loading, logged, error, user }
}
