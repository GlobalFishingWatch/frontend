...
const DataviewTypeDropdown = () => {
  return <div className={fieldStyles.field}>
    ...
    <select className={fieldStyles.value}>
      {Object.entries(Type).map(([t, v]: [string, string]) => 
...
const ResolvedViewParams = ({ params }: any) => {
  ...
  return <Fragment>
    {Object.entries(params).map(
      ([fieldkey, value]: [string, unknown]) => (fieldkey === 'type') ? <DataviewTypeDropdown key={fieldkey} /> : <Field key={fieldkey} keyEditable fieldkey={fieldkey} value={value as string} />
    )}
  </Fragment>
}
...
const ResolvedDatasetParams = ({ params }: any) => {
  ...
  return <Fragment>
    {Object.entries(params).map(
      ([fieldkey, value]: [string, unknown]) => <Field key={fieldkey} fieldkey={fieldkey} value={value as string} />
    )}
  </Fragment>
}
...
const Dataview = () => {
  ...
  return <Fragment>
    ...
    <Section>
      ...
      {dataview.datasetIds && dataview.datasetIds?.map((datasetId) => 
    ...
    <Section>
      ...
      {dataview.resolvedDatasetsParams?.map((resolvedDatasetParams, index) => <ResolvedDatasetParams key={index} params={resolvedDatasetParams} />)}
    </Section>
    <Section>
