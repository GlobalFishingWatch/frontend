...
type HeaderValue<T> = (T extends 'number' ? number : string) | null
function getResponseHeader<T extends 'number' | 'string' = 'string'>({
  response,
  header,
  type,
}: {
  ...
  if (type === 'number') {
    ...
    return (Number.isFinite(parsedValue) ? parsedValue : null) as HeaderValue<T>
  }
  return value as HeaderValue<T>
}
