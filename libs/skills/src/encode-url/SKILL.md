...
3. **Set filters** in `config.filters` — valid filter ids and enum values per dataset are in [references/filters.md](references/filters.md). Flags are ISO3 codes (`ESP`, `FRA`, `CHN`).
   ...
   - **`vms` and national layers bundle several datasets, each with its own filters and values.** A filter only applies when EVERY active dataset supports it with the selected values. Only `flag` and `vessel-groups` are safe everywhere (see filters.md). The encoder enforces this:
     - Some datasets support the filter → it narrows `config.datasets` to them and returns a `warnings` line. Tell the user, e.g. "gear type trawlers is only reported by Norway VMS, so only Norway is shown".
     - None support it, or your own `config.datasets` has one that doesn't → the script fails and lists what each dataset supports. Fix the input and rerun.
     - Different vocabularies for the same concept (Chile `TRAWLERS`, Norway `trawlers`, Brazil `bottom_trawl`) → one instance per dataset, each with its own `config.datasets` + value. Never put both vocabularies in one filter.
