...
Agent skills for GFW applications, shareable across repos. Each skill lives in `src/<skill-name>/`: a `SKILL.md` (instructions for the agent), runnable scripts, reference docs, and its own code — built into its own self-contained bundle so the whole `dist/<skill-name>/` folder can be copied anywhere and just works.

**Prerequisites:** Node 24 (see `.nvmrc` at the repo root) and pnpm (see `packageManager` in the root `package.json`). Skill scripts themselves run on Node >= 24 (`module.registerHooks`).

This package is not consumed by apps in this monorepo. It lives here so the bundles can reuse platform config and `@globalfishingwatch/*` libraries.
...
Copy the built skill folder into the agent's skills directory. Prefer the published npm package — no monorepo clone, no workspace install.
...
**No `.d.ts` is published** — esbuild can't emit them, and a plain `tsc` emit would leak imports of
`@globalfishingwatch/dataviews-client` and the private `@platform/config` into the output.
...
This produces `libs/skills/dist/`: a combined `index.js`, plus one self-contained folder per skill (`encode-url/`, `decode-url/`) — each with its own `index.js` bundle alongside `SKILL.md`, `scripts/`, and `references/`.

Skill scripts run with plain `node` (>= 24) and import their own skill's bundle as `@globalfishingwatch/skills/<skill-name>`, resolved by `scripts/register-gfw-resolver.mjs` from (in order): a sibling `index.js` (dist output), a `../../dist/<skill-name>/index.js` in this monorepo (unbuilt source layout), or — once installed as an npm package — normal `node_modules` resolution via the package's `exports` map.
...
- add its `assets` globs (`SKILL.md`, `scripts/**/*`, `references/**/*`, …) to the `build` target in `project.json`
