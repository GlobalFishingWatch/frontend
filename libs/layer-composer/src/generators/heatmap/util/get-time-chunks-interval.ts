...
export const getTimeChunksInterval = (config: HeatmapAnimatedGeneratorConfig, start: string, end: string) => {
  const availableIntervals = config.availableIntervals ? [config.availableIntervals] : config.sublayers.map(s => s.availableIntervals)
  const omitIntervals: Interval[] = config.mode === HeatmapAnimatedMode.TimeCompare ? ['month', '10days'] : []
