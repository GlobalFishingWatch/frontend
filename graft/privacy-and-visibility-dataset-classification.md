---
name: Privacy and Visibility Dataset Classification
slug: privacy-and-visibility-dataset-classification
type: concept
sources:
...
Utility functions classify datasets as private (via isPrivateDataset), GFW-only (isGFWOnlyDataset), or Skylight (getIsSkylightDataset). Full datasets are filtered out for guest users; private datasets are excluded entirely when guestUser is true. Public datasets are shown to all users, with suffixes (PUBLIC_SUFIX, FULL_SUFIX) appended to labels for clarity in the UI.
