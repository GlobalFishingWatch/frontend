---
name: Dataview Resolution & Filtering
slug: dataview-resolution-filtering
type: system
sources:
...
links:
  - to: dataset-resolution-pipeline
    relation: uses
    description: Calls resolveEndpoint to fetch API URLs for dataview resources
  - to: dataview-url-workspace
    relation: implements
    description: Accepts UrlDataviewInstance objects loaded from URL state
  - to: deck-layer-resource-fetching
    relation: produces
    description: Provides resolved dataset configs and URLs to resource fetchers
  - to: sql-filter-computation
    relation: produces
    description: Generates SQL WHERE clauses from user filters with type-specific handling
generator:
  version: 1
covers:
...
Orchestrates transformation of user-defined dataview instances into fully resolved configurations with merged datasets, computed SQL filters, and resource URLs. Handles multiple dataview categories (Activity, Track, Heatmap, Context) with specialized filter and config accessors.

## Related

- uses [[dataset-resolution-pipeline]] — Calls resolveEndpoint to fetch API URLs for dataview resources
- implements [[dataview-url-workspace]] — Accepts UrlDataviewInstance objects loaded from URL state
- produces [[deck-layer-resource-fetching]] — Provides resolved dataset configs and URLs to resource fetchers
- produces [[sql-filter-computation]] — Generates SQL WHERE clauses from user filters with type-specific handling
