---
name: Fourwings Heatmap Utilities
slug: fourwings-heatmap-utilities
type: system
sources:
  - path: >-
...
links:
  - to: coordinate-filtering-bounds
    relation: implements
    description: >-
      filterCellsByBounds clips geographic data to viewport bounds with
      antimeridian wrapping support via world-copy coordinate offsets
  - to: per-sublayer-visibility-filtering
    relation: implements
    description: >-
      isSublayerValueVisible distinguishes measured zeros from missing/undefined
      data to prevent incorrect ramp extremes
  - to: temporal-aggregation-caching
    relation: implements
    description: >-
      Implements aggregateSublayerValues (sum/avg/directional), aggregateCell,
      compareCell, and sliceCellValues for cross-time aggregation with null vs
      zero distinction
generator:
  version: 1
covers:
...
Comprehensive data processing module for Fourwings heatmap visualization, handling temporal aggregation (sum/average/directional), tile URL construction, coordinate filtering with world-wrapping, frame range mapping, and color domain statistics. Maintains an intervalFramesCache Map (capped at 1000 entries) for memoizing expensive frame calculations.

## Related

- implements [[coordinate-filtering-bounds]] — filterCellsByBounds clips geographic data to viewport bounds with antimeridian wrapping support via world-copy coordinate offsets
- implements [[per-sublayer-visibility-filtering]] — isSublayerValueVisible distinguishes measured zeros from missing/undefined data to prevent incorrect ramp extremes
- implements [[temporal-aggregation-caching]] — Implements aggregateSublayerValues (sum/avg/directional), aggregateCell, compareCell, and sliceCellValues for cross-time aggregation with null vs zero distinction
