...
links:
  - to: asynchronous-font-loading
    relation: implements
    description: >-
      loadDeckFont loads Roboto from Google Fonts and registers via FontFace
      API; renderLayers returns empty until fontLoaded=true to ensure atlas
      availability
  - to: deck-gl-composite-layer-pattern
    relation: uses
    description: >-
      Extends CompositeLayer, defers rendering until font ready; uses
      PaddedCharactersLayer to adjust vertex shaders for SDF font atlas padding
generator:
  version: 1
covers:
...
Composite deck.gl layer for rendering text labels on maps with asynchronous font loading, character padding for SDF fonts, collision detection, and customizable positioning. Extends CompositeLayer wrapping TextLayer with PaddedCharactersLayer for glyph rendering.

## Related

- implements [[asynchronous-font-loading]] — loadDeckFont loads Roboto from Google Fonts and registers via FontFace API; renderLayers returns empty until fontLoaded=true to ensure atlas availability
- uses [[deck-gl-composite-layer-pattern]] — Extends CompositeLayer, defers rendering until font ready; uses PaddedCharactersLayer to adjust vertex shaders for SDF font atlas padding
