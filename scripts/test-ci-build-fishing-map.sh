#!/usr/bin/env bash
# Mirrors .github/actions/build-and-push for fishing-map locally.
# Default: build + load into local Docker (no push). Use --push to match CI fully.
#
# Usage:
#   ./scripts/test-ci-build-fishing-map.sh
#   ./scripts/test-ci-build-fishing-map.sh --push
#   ./scripts/test-ci-build-fishing-map.sh --env staging
#   NX_CLOUD_ACCESS_TOKEN=... ./scripts/test-ci-build-fishing-map.sh
#
...
Build the fishing-map Docker image the same way GitHub Actions does
